...
class FFFingerPrinter {
  ...
  async fingerprint() {
    ...
    if (!this._opts.skipProbing) {
      ...
      this.logger.logDebug(`Probing..`);
      /** @type {ProbeResult} */
      probeResult = JSON.parse((await pwProbe.spawnAsync(false)).stdOut);
    ...
    if (!this._opts.skipFFversions) {
      resultWithHashInfo.hashInfo.fffVersions = await this._getFFFVersions();
    } else {
      this.logger.logInfo('Skipping obtaining ffmpeg/ffprobe versions.');
    }
  ...
  _getFFFVersions() {
    return new Promise(async(resolve, reject) => {
      ...
      let versions = null;
      try {
        versions = await Promise.all([ fffVersionPromise, pwFFmpeg.run(false), pwFFprobe.run(false) ]);
      } catch (e) {
        reject(e);
        return;
      }
      ...
      const fffVersions = {
        ...
        ffmpegVersion: trimFFVersion(versions[1].stdOut),
        ffprobeVersion: trimFFVersion(versions[2].stdOut)
      };
  ...
  _hashAll(rwhi) {
    ...
    const allHashes = rwhi.hashInfo.hashesChapters.map(chapter => { return {
      ...
      .sort((c1, c2) => c1.hash < c2.hash ? -1 : 1)
      .concat(rwhi.hashInfo.hashesStreams.map(stream => { return {
        id: `s:${stream.index}`,
        hash: stream.hash
      }; }).sort((s1, s2) => s1.hash < s2.hash ? -1 : 1));
  ...
  static deterministicToStringObj(obj) {
    if (obj === void 0) {
      ...
      return 'null';
    } else if (Resolve.isPrimitive(obj) || Resolve.isTypeOf(obj, [])) {
      return obj.toString();
